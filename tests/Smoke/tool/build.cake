// .NET Tool runner smoke test for the packed Cake.Grype. The build's Smoke target renders this template into
// artifacts/smoke/tool (filling in @@-placeholders) and runs it with Cake.Tool; it is not meant to run from here.
// No using directives on purpose: the aliases and their namespace imports must work as a script author sees them.
#addin nuget:@@FEED_URI@@/?package=Cake.Grype&version=@@VERSION@@&prerelease
