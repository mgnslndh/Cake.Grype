...
| `release.yml` | push tag `v*` | windows-latest, `environment: Production`, `permissions: contents: write, id-token: write` | same setup and `--target All` (a tag that fails tests or the dogfood gate never publishes) → `NuGet/login@v1` (`user: ${{ secrets.NUGET_USER }}`) → `--target Release` with `NUGET_API_KEY` from the login step and `GITHUB_TOKEN` |
